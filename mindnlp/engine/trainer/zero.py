...
class ZeroHelper:
    ...
    def __init__(
        ...
        if not self.is_parallel and self.zero_stage != 0:
            logger.warning("Not in DATA_PARALLEL, set zero_stage to 0.")
            self.zero_stage = 0
        ...
        if optimizer_offload:
            if isinstance(self.optimizer, nn.AdamWeightDecay):
                ...
                logger.info("Set optimizer run offload.")
            else:
                logger.warning("optimizer_offload only take effect when optimizer is AdamWeightDecay.")
    ...
    def set_zero1_allreduce_fusion_comm_list(self, comm_fusion):
        ...
        logger.info(f"zero1_allreduce_fusion: {allreduce_info}")
    ...
    def set_zero2_reduce_scatter_fusion_comm_list(self, comm_fusion):
        ...
        logger.info(f"zero2_reduce_scatter_fusion: {reduce_scatter_info}")
        logger.info(f"zero2_reduce_scatter_fusion: {allreduce_info}")
    ...
    def set_optimizer_allgather_fusion_comm_list(self, comm_fusion):
        ...
        logger.info(f"optimizer_allgather_fusion: {allgather_info}")
    ...
    def set_dp_allreduce_comm_list(self, comm_fusion):
        ...
        logger.info(f"dp_allreduce_fusion: {dp_allreduce_info}")
    ...
    def get_optimizer_param_tuples(self):
        ...
        for name in self.optimizer._params_list:
            if name in ["_parameters", "parameters"]:
                continue
            logger.debug(f"Add optimizer param_tuples {name}")
        ...
        #                 continue
        #             logger.debug(f"Add optimizer param_tuples {attr}")
    ...
    def split_params(self):
        if self.zero_stage in [1, 2] and self.is_parallel:
            logger.info("Clone optimizer.parameters, will increase memory.")
        ...
        param_tuples = self.get_optimizer_param_tuples()
        for i, param in enumerate(self.optimizer._parameters):
            logger.debug(f"Split optimizer param {param.name} {param.shape}")
            ...
            if self.zero_stage == 3:
                ...
                B = param.shape[0]
            logger.debug(f"Do split with zero_stage {self.zero_stage}")
            if param.parallel_optimizer and B >= self.op_group_size and B % self.op_group_size == 0:
                if self.zero_stage in [1, 2]:
                    ori_shape = param.shape
                    param.assign_value(self.split_param(param))
                    logger.debug(f"Optimizer {param.name} from {ori_shape} to {param.shape}")
                for param_tuple in param_tuples:
                    ori_shape = param_tuple[i].shape
                    param_tuple[i].assign_value(self.split_param(param_tuple[i]))
                    logger.debug(f"Optimizer {param_tuple[i].name} " f"from {ori_shape} to {param_tuple[i].shape}")
    ...
    for name, sub_net in network._cells.items():
        ...
        if sub_net._params:
            for param_name in sub_net._params:
                ...
                logger.warning(f"Set param {param.name} parallel_optimizer False, param shape {param.shape}")
...
def prepare_network(network: nn.Cell, zero_stage: int = 0, op_group: str = None, parallel_modules=None):
    if zero_stage != 3 or _get_parallel_mode() != ParallelMode.DATA_PARALLEL:
        logger.info("No need rewrite network and return original network.")
        return network
    logger.info("Rewrite the network, please wait...")
...
def prepare_ema(ema, zero_stage: int = 0, op_group: str = None):
    ...
    logger.info(f"Split EMA params: rank_id {op_rank_id}, rank_size {op_group_size}.")
    ...
    for param_name in ckpts[0].keys():
        ...
        for i in range(group_size):
            if param_name not in jsons[i]:
                logger.warning(f"param {param_name} not in param_split_info_json, keep ori data.")
                if i:
        ...
        if param_value is None:
            ...
            logger.debug("Merge {param_name} to {param_value.shape}")
...
def get_op_dp_group(op_group=None, dp_group=None):
    if op_group is None:
        logger.warning("Not set zero group, set it to WORLD_COMM_GROUP.")
        op_group = GlobalComm.WORLD_COMM_GROUP
    if op_group != GlobalComm.WORLD_COMM_GROUP and dp_group is None:
        raise ValueError(f"op_group {op_group} and dp_group {dp_group} not full network hccl group coverage")
    return op_group, dp_group
